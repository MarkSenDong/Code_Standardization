...
% This script allow to obtain a vector for brain measures (thickness,
% surface etc), based on fsaverage.
% The number of points of the vector depends on fsaverage: i.e. fsaverage5
% gives a vector of 10242 points for each hemisphere; fsaverage6 gives
% 40962
% ATTENTION: before running any script it is necessary to add the freesurfer matlab
% path: addpath /opt/freesurfer/freesurfer_v5.3/matlab
...
for i=1:numel(aux_name)
    ...
    subfile = sprintf('%s/%s/surf/lh.thickness_fsa5.mgh',subdir,id{i}); %the lh subdirectory where smoothed mgh files are stored
